...
while true; do
  ...
  ## --------------- Docker Containers ------------------ ##
  ## If you want to add docker containers to the subdomain list
  ## Iterate over Docker container IDs (You need to add and ENV variable VIRTUAL_HOST)
  while IFS= read -r container_id; do
    if docker inspect "$container_id" | jq -e '.[].Config.Env[] | select(startswith("VIRTUAL_HOST="))' >/dev/null; then
      container_name=$(docker inspect -f '{{.Name}}' "$container_id" | sed 's/^\///') # Get the container name
      filters+=("$container_name")                                                    # Add container name to the array
    fi
  done < <(docker ps -q) # Get the list of running container IDs
  ## --------------- //////////////// ------------------ ##
